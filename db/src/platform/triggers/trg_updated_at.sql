-- db/src/platform/triggers/trg_updated_at.sql
...
BEGIN
    FOR t IN 
        ...
        WHERE c.column_name = 'updated_at' AND c.table_schema = 'platform'
          AND EXISTS (SELECT 1 FROM information_schema.tables tb
                      WHERE tb.table_schema = 'platform' AND tb.table_name = c.table_name
