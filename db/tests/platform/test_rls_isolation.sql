...
-- 运行方式：pg_prove -U app_owner -d app_db db/tests/platform/test_rls_isolation.sql
BEGIN;
SELECT plan(14);
...
-- ============================================================
SELECT results_eq(
    $$ SELECT count(*) FROM pg_policies WHERE tablename = 'users' AND schemaname = 'platform' $$,
    ARRAY[1::bigint],
...
-- ============================================================
SELECT results_eq(
    $$ SELECT count(*) FROM pg_class c JOIN pg_namespace n ON c.relnamespace = n.oid WHERE c.relname = 'users' AND n.nspname = 'platform' AND c.relrowsecurity = true $$,
    ARRAY[1::bigint],
    'users 表已启用 RLS'
);

SELECT results_eq(
    $$ SELECT count(*) FROM pg_class c JOIN pg_namespace n ON c.relnamespace = n.oid WHERE c.relname = 'role' AND n.nspname = 'platform' AND c.relrowsecurity = true $$,
    ARRAY[1::bigint],
    'role 表已启用 RLS'
);
...
SELECT ok(
    (SELECT data_type = 'text' FROM information_schema.routines
     WHERE routine_name = 'current_tenant_id' AND specific_schema = 'platform'),
