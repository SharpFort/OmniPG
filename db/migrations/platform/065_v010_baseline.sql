...
COMMENT ON COLUMN platform.audit_log.log_type IS '日志类型: data_change(触发器差异日志,默认) / operate(业务操作) / login(登录) / exception(异常) / event(事件) / open_api(开放接口)';
COMMENT ON COLUMN platform.audit_log.module IS '业务模块（order/user/...）';
COMMENT ON COLUMN platform.audit_log.action IS '操作标识（order.approve）';
COMMENT ON COLUMN platform.audit_log.target_type IS '操作对象类型';
COMMENT ON COLUMN platform.audit_log.target_id IS '操作对象 ID';
COMMENT ON COLUMN platform.audit_log.result IS '执行结果: success / fail';
COMMENT ON COLUMN platform.audit_log.region IS 'IP 归属地（ip2region: 国家|省|市|ISP）';
CREATE TABLE IF NOT EXISTS platform.cron_job_log (
...
COMMENT ON TABLE platform.cron_job_log IS 'pg_cron 任务执行日志';
...
COMMENT ON TABLE platform.department IS '部门组织架构表，按租户隔离';
COMMENT ON COLUMN platform.department.tenant_id IS '所属租户，租户间部门数据隔离';
COMMENT ON COLUMN platform.department.parent_id IS '上级部门 ID，NULL 表示根部门';
COMMENT ON COLUMN platform.department.created_by IS '创建者用户 ID';
COMMENT ON COLUMN platform.department.updated_by IS '最后修改者用户 ID';
COMMENT ON COLUMN platform.department.deleted_by IS '删除者用户 ID';
CREATE TABLE IF NOT EXISTS platform.dict_data (
...
COMMENT ON TABLE platform.dict_data IS '字典数据项';
CREATE TABLE IF NOT EXISTS platform.dict_type (
...
COMMENT ON TABLE platform.dict_type IS '字典类型（全局 + 租户两级）';
COMMENT ON COLUMN platform.dict_type.tenant_id IS 'NULL=全局字典（所有租户共享）；非 NULL=租户字典（RLS 按 claims 过滤）';
CREATE TABLE IF NOT EXISTS platform.iam_menu (
    ...
    created_by text,
    updated_by text,
    menu_type platform.iam_menu_type DEFAULT 'menu'::platform.iam_menu_type NOT NULL,
    api_code text,
    component text,
    ...
    CONSTRAINT iam_menu_button_nav_null_check CHECK (((menu_type <> 'button'::platform.iam_menu_type) OR ((router IS NULL) AND (component IS NULL)))),
    CONSTRAINT iam_menu_button_perms_check CHECK (((menu_type <> 'button'::platform.iam_menu_type) OR ((api_code IS NOT NULL) AND (TRIM(BOTH FROM api_code) <> ''::text)))),
...
COMMENT ON COLUMN platform.iam_menu.component IS '前端组件路径（路由渲染，仅 menu 类型使用）';
COMMENT ON COLUMN platform.iam_menu.is_visible IS '是否显示（目录/菜单显隐控制）';
COMMENT ON COLUMN platform.iam_menu.remark IS '备注（管理端展示）';
COMMENT ON COLUMN platform.iam_menu.route_name IS '路由名称（Vue Router name，英文唯一；前端 addRoute 用）';
COMMENT ON COLUMN platform.iam_menu.is_link IS '是否外链（新窗口打开；menu_type=link 时自动置 true）';
COMMENT ON COLUMN platform.iam_menu.is_iframe IS '是否 iframe 内嵌（path 为内嵌 URL）';
COMMENT ON COLUMN platform.iam_menu.redirect IS '目录重定向路径（noRedirect 表示不重定向）';
COMMENT ON COLUMN platform.iam_menu.is_cache IS '是否缓存页面（keep-alive，默认 true；057 由 keep_alive 改名——语义对标 SharpFort IsCache/RuoYi is_cache + iam_menu 布尔列 is_ 前缀命名统一）';
COMMENT ON COLUMN platform.iam_menu.api_url IS 'API 端点路径（原 iam_api.path，055 单表化 D1；仅 button 行使用；SharpFort ApiUrl 借鉴；约定以 / 开头不含 {}，RPC 层软校验 P2）';
COMMENT ON COLUMN platform.iam_menu.api_method IS 'API 端点方法（原 iam_api.method，055 单表化 D1；api_url 非空时必填，值域 GET/POST/PUT/PATCH/DELETE/HEAD/OPTIONS/*；SharpFort ApiMethod 借鉴）';
...
COMMENT ON TABLE platform.login_log IS '登录日志（业务端安全审计镜像：Logto 审计日志无租户隔离/会被清理，业务端保留长期记录）';
CREATE TABLE IF NOT EXISTS platform."position" (
    id uuid DEFAULT uuidv7() NOT NULL,
...
COMMENT ON TABLE platform."position" IS '岗位表（树形，租户隔离）。岗位=职级/职务维度，与权限无关（权限用角色）';
COMMENT ON COLUMN platform."position".parent_id IS '上级岗位 ID，NULL 表示根岗位';
CREATE TABLE IF NOT EXISTS platform.user_position (
...
COMMENT ON TABLE platform.user_position IS '用户-岗位关联（多对多，租户隔离）';
CREATE TABLE IF NOT EXISTS platform.user_profile (
    user_id text NOT NULL,
    tenant_id text,
    dept_id uuid,
...
COMMENT ON TABLE platform.user_profile IS '用户个人资料（应用自有扩展，用户可编辑）：users 为 Logto 认证镜像只读，user_profile 承载 nickname/头像/生日/爱好/住址等个人信息；RLS 本人/超管可写（rls_policies.sql profile_tenant_policy）';
COMMENT ON COLUMN platform.user_profile.user_id IS 'Logto 用户 id（users.id 1:1；主键即外键，ON DELETE CASCADE）';
COMMENT ON COLUMN platform.user_profile.tenant_id IS '租户 organization_id（017 约定；NULL=全局个人资料）';
COMMENT ON COLUMN platform.user_profile.dept_id IS '部门归属（current_user_dept_id 依赖；department 表）';
COMMENT ON COLUMN platform.user_profile.nickname IS '昵称（应用自定义显示名；users.name 为 Logto 权威显示名，不重复）';
COMMENT ON COLUMN platform.user_profile.avatar_url IS '头像 URL（空=默认头像，前端兜底）';
COMMENT ON COLUMN platform.user_profile.gender IS '性别（gender 枚举：male/female/other/prefer_not_to_say）';
COMMENT ON COLUMN platform.user_profile.birthday IS '生日（隐私字段；前端按需脱敏）';
COMMENT ON COLUMN platform.user_profile.bio IS '个人简介（≤500 字）';
COMMENT ON COLUMN platform.user_profile.location IS '所在地/住址（自由文本）';
COMMENT ON COLUMN platform.user_profile.hobbies IS '爱好（标签数组，如 {"篮球","摄影"}）';
COMMENT ON COLUMN platform.user_profile.website IS '个人主页 URL';
...
CREATE TABLE IF NOT EXISTS platform.iam_role_data_scope (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by text,
    CONSTRAINT iam_role_data_scope_dept_consistency CHECK ((((scope_type = 'custom'::platform.scope_type) AND (dept_id IS NOT NULL)) OR ((scope_type <> 'custom'::platform.scope_type) AND (dept_id IS NULL))))
);
COMMENT ON TABLE platform.iam_role_data_scope IS '角色数据范围（授权判定数据；RLS 部门维度过滤的依据）';
COMMENT ON COLUMN platform.iam_role_data_scope.scope_type IS '数据范围: all=全部 / dept_and_child=本部门及以下 / self=仅本人 / custom=自定义部门';
COMMENT ON COLUMN platform.iam_role_data_scope.dept_id IS 'custom 时指定部门（一个角色可多行）；其余类型恒 NULL（约束保证）';
CREATE TABLE IF NOT EXISTS platform.ip_geolite2_blocks (
...
COMMENT ON TABLE platform.ip_geolite2_blocks IS 'GeoLite2-City-Blocks-IPv4.csv staging（导入管道）';
CREATE TABLE IF NOT EXISTS platform.ip_geolite2_city (
    network cidr NOT NULL,
    geoname_id bigint,
    latitude double precision,
    longitude double precision,
...
COMMENT ON TABLE platform.ip_geolite2_city IS 'GeoLite2-City 离线库（Blocks×Locations join 导入，只读）；全球覆盖含经纬度/时区；ip2region 未命中或 IPv6 时兜底';
CREATE TABLE IF NOT EXISTS platform.ip_geolite2_locations (
...
COMMENT ON TABLE platform.ip_geolite2_locations IS 'GeoLite2-City-Locations-zh-CN.csv staging（导入管道）';
CREATE TABLE IF NOT EXISTS platform.ip_region_v4 (
...
COMMENT ON TABLE platform.ip_region_v4 IS 'IP 归属地离线库（ip2region v4 数据导入，只读）';
CREATE SEQUENCE IF NOT EXISTS platform.sys_cron_log_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE platform.sys_cron_log_id_seq OWNED BY platform.cron_job_log.id;
CREATE SEQUENCE IF NOT EXISTS platform.sys_login_log_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER SEQUENCE platform.sys_login_log_id_seq OWNED BY platform.login_log.id;
CREATE TABLE IF NOT EXISTS platform.webhook_event_log (
    id uuid DEFAULT uuidv7() NOT NULL,
    hook_id text,
    event text NOT NULL,
...
COMMENT ON TABLE platform.webhook_event_log IS 'Logto webhook 事件日志（N6：同步链路可观测；payload 留存供审计/重放；保留 90 天）';
COMMENT ON COLUMN platform.webhook_event_log.result IS '处理结果：received（落库未完成）/ success / error（同步失败）/ ignored（未知事件）';
ALTER TABLE ONLY platform.cron_job_log ALTER COLUMN id SET DEFAULT nextval('platform.sys_cron_log_id_seq'::regclass);
ALTER TABLE ONLY platform.login_log ALTER COLUMN id SET DEFAULT nextval('platform.sys_login_log_id_seq'::regclass);
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'iam_menu_pkey') THEN
        ALTER TABLE ONLY platform.iam_menu ADD CONSTRAINT iam_menu_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'position_pkey') THEN
        ALTER TABLE ONLY platform."position" ADD CONSTRAINT position_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_audit_log_pkey') THEN
        ALTER TABLE ONLY platform.audit_log ADD CONSTRAINT sys_audit_log_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_config_config_key_key') THEN
        ALTER TABLE ONLY platform.app_config ADD CONSTRAINT sys_config_config_key_key UNIQUE (config_key);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_config_pkey') THEN
        ALTER TABLE ONLY platform.app_config ADD CONSTRAINT sys_config_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_cron_log_pkey') THEN
        ALTER TABLE ONLY platform.cron_job_log ADD CONSTRAINT sys_cron_log_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_department_pkey') THEN
        ALTER TABLE ONLY platform.department ADD CONSTRAINT sys_department_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_dict_data_pkey') THEN
        ALTER TABLE ONLY platform.dict_data ADD CONSTRAINT sys_dict_data_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_dict_type_pkey') THEN
        ALTER TABLE ONLY platform.dict_type ADD CONSTRAINT sys_dict_type_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_dict_type_tenant_id_dict_name_key') THEN
        ALTER TABLE ONLY platform.dict_type ADD CONSTRAINT sys_dict_type_tenant_id_dict_name_key UNIQUE (tenant_id, dict_name);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_login_log_pkey') THEN
        ALTER TABLE ONLY platform.login_log ADD CONSTRAINT sys_login_log_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_position_pkey') THEN
        ALTER TABLE ONLY platform.user_position ADD CONSTRAINT user_position_pkey PRIMARY KEY (user_id, position_id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_profile_pkey') THEN
        ALTER TABLE ONLY platform.user_profile ADD CONSTRAINT user_profile_pkey PRIMARY KEY (user_id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'webhook_event_log_pkey') THEN
        ALTER TABLE ONLY platform.webhook_event_log ADD CONSTRAINT webhook_event_log_pkey PRIMARY KEY (id);
    END IF;
END $$;
CREATE INDEX IF NOT EXISTS idx_audit_created ON platform.audit_log USING btree (created_at);
CREATE INDEX IF NOT EXISTS idx_audit_logtype ON platform.audit_log USING btree (log_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_audit_operation ON platform.audit_log USING btree (operation);
CREATE INDEX IF NOT EXISTS idx_audit_tenant ON platform.audit_log USING btree (tenant_id);
CREATE INDEX IF NOT EXISTS idx_audit_user ON platform.audit_log USING btree (user_id);
CREATE INDEX IF NOT EXISTS idx_config_key ON platform.app_config USING btree (config_key);
CREATE INDEX IF NOT EXISTS idx_config_public ON platform.app_config USING btree (is_public) WHERE (is_public = true);
CREATE INDEX IF NOT EXISTS idx_dept_parent ON platform.department USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_dept_tenant ON platform.department USING btree (tenant_id);
CREATE INDEX IF NOT EXISTS idx_dict_data_name ON platform.dict_data USING btree (dict_name, sort_no);
CREATE INDEX IF NOT EXISTS idx_iam_menu_api_code ON platform.iam_menu USING btree (api_code);
CREATE UNIQUE INDEX IF NOT EXISTS idx_iam_menu_api_url_method ON platform.iam_menu USING btree (api_url, api_method) WHERE (api_url IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_login_log_user ON platform.login_log USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_position_parent ON platform."position" USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_position_tenant ON platform."position" USING btree (tenant_id);
CREATE INDEX IF NOT EXISTS idx_user_position_pos ON platform.user_position USING btree (position_id);
CREATE INDEX IF NOT EXISTS idx_user_position_tenant ON platform.user_position USING btree (tenant_id);
CREATE INDEX IF NOT EXISTS idx_user_profile_tenant ON platform.user_profile USING btree (tenant_id);
CREATE INDEX IF NOT EXISTS idx_wev_created ON platform.webhook_event_log USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_wev_event ON platform.webhook_event_log USING btree (event);
CREATE INDEX IF NOT EXISTS idx_wev_hook ON platform.webhook_event_log USING btree (hook_id);
CREATE INDEX IF NOT EXISTS idx_wev_result ON platform.webhook_event_log USING btree (result);
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'iam_menu_parent_id_fkey') THEN
        ALTER TABLE ONLY platform.iam_menu ADD CONSTRAINT iam_menu_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES platform.iam_menu(id) ON DELETE SET NULL;
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'iam_role_data_scope_dept_id_fkey') THEN
        ALTER TABLE ONLY platform.iam_role_data_scope ADD CONSTRAINT iam_role_data_scope_dept_id_fkey FOREIGN KEY (dept_id) REFERENCES platform.department(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'iam_role_menu_menu_id_fkey') THEN
        ALTER TABLE ONLY platform.iam_role_menu ADD CONSTRAINT iam_role_menu_menu_id_fkey FOREIGN KEY (menu_id) REFERENCES platform.iam_menu(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'position_parent_id_fkey') THEN
        ALTER TABLE ONLY platform."position" ADD CONSTRAINT position_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES platform."position"(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sys_department_parent_id_fkey') THEN
        ALTER TABLE ONLY platform.department ADD CONSTRAINT sys_department_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES platform.department(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_position_position_id_fkey') THEN
        ALTER TABLE ONLY platform.user_position ADD CONSTRAINT user_position_position_id_fkey FOREIGN KEY (position_id) REFERENCES platform."position"(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
