...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'iam_role_pkey') THEN
        ALTER TABLE ONLY public.role ADD CONSTRAINT iam_role_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'organization_role_pkey') THEN
        ALTER TABLE ONLY public.organization_role ADD CONSTRAINT organization_role_pkey PRIMARY KEY (id);
    END IF;
END $$;
DO $$ BEGIN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_role_pkey') THEN
        ALTER TABLE ONLY public.user_role ADD CONSTRAINT user_role_pkey PRIMARY KEY (user_id, organization_id, role_code);
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_tenants_pkey') THEN
        ALTER TABLE ONLY public.user_tenants ADD CONSTRAINT user_tenants_pkey PRIMARY KEY (organization_id, user_id);
    END IF;
END $$;
DO $$ BEGIN
    ...
    END IF;
END $$;
CREATE UNIQUE INDEX IF NOT EXISTS idx_org_role_name ON public.organization_role USING btree (name);
CREATE UNIQUE INDEX IF NOT EXISTS idx_role_name ON public.role USING btree (name);
CREATE INDEX IF NOT EXISTS idx_role_type ON public.role USING btree (type);
CREATE INDEX IF NOT EXISTS idx_tenants_name ON public.tenants USING btree (name);
CREATE INDEX IF NOT EXISTS idx_users_is_suspended ON public.users USING btree (is_suspended);
CREATE INDEX IF NOT EXISTS idx_users_primary_email ON public.users USING btree (primary_email);
CREATE INDEX IF NOT EXISTS idx_users_username ON public.users USING btree (username);
CREATE INDEX IF NOT EXISTS idx_ut_org ON public.user_tenants USING btree (organization_id);
CREATE INDEX IF NOT EXISTS idx_ut_user ON public.user_tenants USING btree (user_id);
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_role_role_id_fk') THEN
        ALTER TABLE ONLY public.user_role ADD CONSTRAINT user_role_role_id_fk FOREIGN KEY (role_id) REFERENCES public.role(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_tenants_organization_id_fkey') THEN
        ALTER TABLE ONLY public.user_tenants ADD CONSTRAINT user_tenants_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES public.tenants(id) ON DELETE CASCADE;
    END IF;
END $$;
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'user_tenants_user_id_fkey') THEN
        ALTER TABLE ONLY public.user_tenants ADD CONSTRAINT user_tenants_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
    END IF;
END $$;
