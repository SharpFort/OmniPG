...
GRANT SELECT ON user_role TO super_admin;

-- 镜像表（009 只授 authenticated；super_admin/业务角色须可读——视图为 INVOKER 权限）
-- N4（2026-08-11）: REVOKE 写授权（旧 GRANT ALL）——镜像只读原则，
--   写入通道仅 sync_* webhook / ensure_user JIT / 对账任务（均 SECURITY DEFINER）
REVOKE ALL ON users, tenants, role, user_tenants FROM super_admin;
GRANT SELECT ON users, tenants, role, user_tenants TO super_admin;
