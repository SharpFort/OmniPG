...
BEGIN
    ...
    -- N7（2026-08-11）: users 镜像完全由 webhook（User.*）维护，JIT 仅缺失补建。
    --   claims 不含 username/name/avatar（claims 脚本仅注入 roles/pg_role），
    --   旧版 ON CONFLICT DO UPDATE 以空串覆盖 webhook 权威值 → 登录后用户名/头像被清空；
    --   is_suspended 不由 JIT 管理（封禁经 User.SuspensionStatus.Updated 同步，P1 D7），
    --   补建行依赖默认 false。
    ...
    --   旧版 ON CONFLICT DO UPDATE SET tenant_id=EXCLUDED.tenant_id 使多组织用户归属漂移
