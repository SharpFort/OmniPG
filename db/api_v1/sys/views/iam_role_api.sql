...
DROP VIEW IF EXISTS api_v1_sys.iam_role_api CASCADE;
CREATE OR REPLACE VIEW api_v1_sys.iam_role_api AS
SELECT r.id AS role_id, ra.api_id, ra.created_at, ra.created_by
FROM iam_role_api ra
JOIN role r ON r.role_code = ra.role_code;
COMMENT ON VIEW api_v1_sys.iam_role_api IS '角色-API 关联视图（Logto 镜像：iam_role_api）';
