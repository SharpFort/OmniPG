...
CREATE OR REPLACE FUNCTION api_v1_sys.export_csv(
    p_table_name text,
...
BEGIN
    ...
    IF NOT (p_table_name = ANY(v_valid_tables)) THEN
        RAISE EXCEPTION 'Table/view % not found in api_v1_sys schema. Valid: %', 
            p_table_name, array_to_string(v_valid_tables, ', ') USING ERRCODE = 'P0001';
    END IF;
    
    -- 构造查询
    IF p_filter = '' THEN
        v_query := format('COPY (SELECT %s FROM api_v1_sys.%I) TO STDOUT WITH CSV HEADER',
                          p_columns, p_table_name);
    ELSE
        v_query := format('COPY (SELECT %s FROM api_v1_sys.%I WHERE %s) TO STDOUT WITH CSV HEADER',
    ...
    -- 实际导出建议：前端直接 GET /api_v1_sys/view_name?select=col1,col2 后自行拼 CSV
    RETURN format('Use GET /api_v1_sys/%s?select=%s to fetch data as JSON, then convert to CSV in frontend.',
                  p_table_name, p_columns);
END;
$$;
COMMENT ON FUNCTION api_v1_sys.export_csv(text, text, text) IS '通用导出提示：PostgREST 不支持流式 COPY，建议前端通过标准 GET 接口获取 JSON 后转换';
