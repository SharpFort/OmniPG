-- api_v1/platform/rpc/rpc_create_menu.sql
-- FUNCTION: api_v1_platform.rpc_create_menu（17 号文档归位：迁移 057_iam_menu_keep_alive_rename_is_cache.sql 删定义段，本文件为唯一权威）
-- 回放终态: 057_iam_menu_keep_alive_rename_is_cache.sql；幂等写法（§9 模板）

CREATE OR REPLACE FUNCTION api_v1_platform.rpc_create_menu(
...
DECLARE v_id uuid;
BEGIN
    IF NOT has_permission('platform:menu:create') THEN
...
END $$;
GRANT EXECUTE ON FUNCTION api_v1_platform.rpc_create_menu(text, uuid, text, text, text, text, text, int, boolean, text, text, boolean, boolean, text, boolean, text, text, boolean) TO authenticated;
