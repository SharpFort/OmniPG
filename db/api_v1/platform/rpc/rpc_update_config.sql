-- db/api_v1/platform/rpc/rpc_update_config.sql
...
CREATE OR REPLACE FUNCTION api_v1_platform.update_config(
    p_config_key text,
    p_config_value text
)
RETURNS boolean
...
AS $$
BEGIN
    IF NOT has_permission('platform:config:write') THEN
...
END;
$$;
COMMENT ON FUNCTION api_v1_platform.update_config(text, text) IS '更新系统配置（public:config:write；029 补门槛，035 源文件同步）';
GRANT EXECUTE ON FUNCTION api_v1_platform.update_config(text, text) TO authenticated;
