-- api_v1/platform/rpc/rpc_delete_menu.sql
-- FUNCTION: api_v1_platform.rpc_delete_menu（17 号文档归位：迁移 024_admin_crud_rpc.sql 删定义段，本文件为唯一权威）
-- 回放终态: 024_admin_crud_rpc.sql；幂等写法（§9 模板）

CREATE OR REPLACE FUNCTION api_v1_platform.rpc_delete_menu(p_id uuid)
RETURNS json
LANGUAGE plpgsql SECURITY DEFINER SET search_path = platform, ext, pg_temp AS $$
BEGIN
    IF NOT has_permission('platform:menu:delete') THEN
...
END $$;
GRANT EXECUTE ON FUNCTION api_v1_platform.rpc_delete_menu(uuid) TO authenticated;
