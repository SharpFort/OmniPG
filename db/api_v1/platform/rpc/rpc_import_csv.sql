-- db/api_v1/platform/rpc/rpc_import_csv.sql
...
CREATE OR REPLACE FUNCTION api_v1_platform.import_csv(
    p_table_name text,
    p_data jsonb,
...
DECLARE
    ...
    v_sql      text;
BEGIN
    PERFORM require_permission('platform:import');
    ...
    LOOP
        BEGIN
            ...
            v_sql := format('INSERT INTO api_v1_platform.%I (%s) ' ||
                            'SELECT %s FROM jsonb_populate_record(NULL::api_v1_platform.%I, $1)',
...
END;
$$;
COMMENT ON FUNCTION api_v1_platform.import_csv(text, jsonb, boolean) IS '通用导入（035 重写：显式业务表白名单 + jsonb_populate_record 参数化列子集；public:import）';
GRANT EXECUTE ON FUNCTION api_v1_platform.import_csv(text, jsonb, boolean) TO authenticated;
