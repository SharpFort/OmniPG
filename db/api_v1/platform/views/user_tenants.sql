...
-- 034: 由 api_v1_platform.user_role 更名而来（消除与 platform.user_role 表同名冲突）
-- 背景: T7 时代 user_role 视图 = user_tenants 投影（组织成员关系）；
--       024 新建 platform.user_role 表（用户↔Logto 全局角色分配镜像）后同名不同物。
...
COMMENT ON VIEW api_v1_platform.user_tenants IS '用户-组织成员关系视图（Logto 镜像：user_tenants；034 由 user_role 更名，消除与 platform.user_role 表同名冲突）';
