...
-- 061: tenants 无软删（Logto 删除=行删除），total_tenants 直接计数
-- 来源: 20260707000015_system_management_api.sql → T7 适配 → 055 → 061
...
SELECT
    ...
    now() AS stats_time;
COMMENT ON VIEW api_v1_platform.v_system_stats IS '系统统计面板视图（单行汇总，Logto 镜像表；055: total_apis 口径=button 行带端点；061 tenants 直计）';
