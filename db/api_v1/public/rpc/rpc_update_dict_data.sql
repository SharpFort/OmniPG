-- api_v1/public/rpc/rpc_update_dict_data.sql
-- FUNCTION: api_v1_public.rpc_update_dict_data（17 号文档归位：迁移 024_admin_crud_rpc.sql 删定义段，本文件为唯一权威）
-- 回放终态: 024_admin_crud_rpc.sql；幂等写法（§9 模板）

CREATE OR REPLACE FUNCTION api_v1_public.rpc_update_dict_data(
...
END $$;
GRANT EXECUTE ON FUNCTION api_v1_public.rpc_update_dict_data(uuid, text, text, text, boolean, int, boolean) TO authenticated;
