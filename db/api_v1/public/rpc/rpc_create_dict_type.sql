-- api_v1/public/rpc/rpc_create_dict_type.sql
-- FUNCTION: api_v1_public.rpc_create_dict_type（17 号文档归位：迁移 024_admin_crud_rpc.sql 删定义段，本文件为唯一权威）
-- 回放终态: 024_admin_crud_rpc.sql；幂等写法（§9 模板）

CREATE OR REPLACE FUNCTION api_v1_public.rpc_create_dict_type(
...
END $$;
GRANT EXECUTE ON FUNCTION api_v1_public.rpc_create_dict_type(text, text, boolean, int) TO authenticated;
