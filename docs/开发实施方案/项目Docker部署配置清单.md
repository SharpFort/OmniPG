...
all:
  vars:
    ...
    pg_packages:
      - postgresql-18
      - postgresql-18-pgtap
      # - postgresql-18-plpython3  — 后续需要时再启用
      - postgresql-contrib
    ...
    pg_extensions:
      ...
      - pg_graphql
      - pg_cron
      # plpython3u — 后续需要时再安装（当前 JWT 由 Casdoor 签发，不使用 PL/Python）
    ...
    pgsql:
      ...
      vars:
        ...
        pg_users:
          ...
          - name: web_anon
            # NOLOGIN 角色，无需密码（PostgREST 角色切换用）
        pg_hba_rules:
          - type: host
            database: all
            user: all
...
services:
  ...
  postgrest:
    ...
    restart: unless-stopped
    environment:
      PGRST_DB_URI: postgres://authenticator:authenticator_dev_pass@host.docker.internal:6432/app_db?sslmode=disable
      PGRST_DB_SCHEMAS: "api_v1"
      PGRST_DB_ANON_ROLE: web_anon
      PGRST_DB_EXTRA_SEARCH_PATH: "public"
      PGRST_JWT_SECRET: ${JWKS_JSON:-{"keys":[{"kty":"RSA","kid":"casdoor-dev","use":"sig","alg":"RS256","n":"REPLACE_WITH_CASDOOR_JWKS_N","e":"AQAB"}]}}
  ...
  casdoor:
    ...
    environment:
      driverName: postgres
      dataSourceName: "user=casdoor password=${CASDOOR_DB_PASSWORD:-casdoor_dev_pass} host=host.docker.internal port=6432 sslmode=disable dbname=casdoor"
      runMode: dev
    volumes:
...
浏览器 (Windows)
    ...
    │                       │
    │                       ▼
    │                   PostgreSQL (Pigsty 原生，端口 5432 ← pgBouncer 6432 代理)
    │
...
| Swagger UI | `localhost:8082` (WSL2 → Windows) | Docker 端口映射 |
...
> **说明：** 采用 Pigsty + Docker 混合架构后，许多配置（Pgbouncer、APISIX、.env、初始化脚本）已纳入 01 文档的对应章节。本文档仅列出差异部分，完整配置请引用 01 文档。

### 6.1 配置文件引用

| 配置文件 | 位置 | 说明 |
|:---|:---|:---|
| `pgbouncer.ini` | 01-环境搭建 §3.2 | Pgbouncer 连接池配置（pool_mode=session，端口 6432） |
| `userlist.txt` | 01-环境搭建 §3.2 | Pgbouncer 用户认证（app_owner / authenticator / casdoor） |
| `apisix/config.yaml` | 01-环境搭建 §3.3 | APISIX 基础配置（etcd 连接、管理员密钥） |
| `apisix/apisix.yaml` | 01-环境搭建 §3.4 | APISIX 路由占位（后续阶段完善） |
| `.env` | 01-环境搭建 §3.5 | 环境变量文件（JWT_SECRET、CASDOOR_DB_PASSWORD 等） |

### 6.2 数据库初始化脚本引用

| 脚本 | 位置 | 说明 |
|:---|:---|:---|
| `01-extensions.sql` | 01-环境搭建 §4.1 | 扩展安装：pgcrypto / pgsodium / pg_net / pgaudit / pg_graphql / pg_cron / pgtap |
| `02-schemas.sql` | 01-环境搭建 §4.2 | 业务 Schema 和角色：api_v1、web_anon、authenticated、authenticator |
| `03-casdoor-db.sql` | 01-环境搭建 §4.3 | Casdoor 数据库和用户创建（需容器首次启动后手动执行） |

---

## 七、PostgREST + OpenAPI/Swagger 配置说明
...
| `http://localhost:8082/` | Swagger UI 可视化界面 |

---

## 八、Policy Syncer — 功能回顾
...
| # | 验收项 | 验证命令/方法 | 预期结果 |
|:---:|:---|:---|:---|
| D1 | 所有 Docker 容器启动 | `docker compose ps` | 全部 `running/healthy` |
| D2 | APISIX 可访问 | `curl http://localhost:9080/apisix/status` | 200 OK |
| D3 | PostgREST 响应 | `curl http://localhost:3000/` | 返回 OpenAPI JSON |
| D4 | Casdoor 健康 | `curl http://localhost:8000/api/health` | `{"status":"ok"}` |
| D5 | Swagger UI | 浏览器打开 `http://localhost:8082` | Swagger 页面加载 |
| D6 | Policy Syncer 健康 | `curl http://localhost:8080/healthz` | `healthy` |
| D7 | PG 可连接（pgBouncer） | `psql -h localhost -p 6432 -U app_owner -d app_db -c "SELECT 1"` | 1 |
| D8 | PG 可连接（直连） | `psql -h localhost -U app_owner -d app_db -c "SELECT version();"` | PG 18 |
| D9 | Redis 可连接 | `redis-cli -h localhost -p 6379 ping` | PONG |
| D10 | etcd 健康 | `etcdctl endpoint health` | is healthy |
| D11 | Grafana（Pigsty） | 浏览器打开 `http://localhost:3001` | Grafana 登录页 |
| D12 | VictoriaMetrics | `curl http://localhost:8428/health` | OK |
| D13 | Docker Compose 网络 | `docker network inspect app-net` | 包含全部 5 个容器 |
| D14 | Syncer→APISIX 连通 | `docker exec policy-syncer curl -s http://apisix:9180/apisix/status` | 200 OK |
...
| 问题 | 可能原因 | 解决方案 |
|:---|:---|:---|
| **端口冲突** | 宿主机端口被占用 | `netstat -ano \| findstr :<端口号>` 查看占用进程；修改 `docker-compose.yml` 端口映射 |
| **容器启动失败** | 镜像拉取失败 | `docker compose pull` 重新拉取；检查网络连接 |
| **Pgbouncer 连接失败** | 配置文件路径错误 | 确认 `pgbouncer.ini` 和 `userlist.txt` 存在且语法正确 |
| **PostgREST 连接 DB 失败** | pgBouncer 未就绪 | 等待 pgBouncer 健康后重试；检查 `pgbouncer.ini` 中 `pool_mode = session` |
| **Casdoor 无法启动** | 数据库未创建 | 执行 `docker exec -i app-postgres psql -U app_owner -d app_db < db/init/03-casdoor-db.sql` |
| **APISIX 无法连接 etcd** | etcd 未启动 | `docker compose restart etcd`；检查 `config.yaml` 中 etcd host 配置 |
| **Syncer 同步失败** | DB 连接错误 | 检查 `DB_HOST`、`DB_PORT`、`DB_PASSWORD` 环境变量；确认 pgBouncer 已启动 |
| **Swagger UI 空白** | API_URL 配置错误 | 确认 `API_URL` 指向 `http://postgrest:3000/`（容器网络内地址） |
| **Grafana 无法访问** | 端口冲突 | 确认 3001 端口未被占用；检查 Pigsty INFRA 模块状态 |
| **WSL2 网络不通** | Docker Desktop 网络隔离 | 确认 Docker Desktop 使用 WSL2 后端；重启 WSL：`wsl --shutdown` |
| **JWT 验证失败** | JWKS 配置错误 | 确认 `JWKS_JSON` 环境变量包含有效的 RSA 公钥；检查 Casdoor JWKS 端点 |
| **pg_graphql 扩展缺失** | 未安装 | 进入 PG 容器：`CREATE EXTENSION pg_graphql;` |
...
| **3001** | Grafana | Pigsty INFRA 模块 | 可视化仪表盘 |
...
| **8080** | Policy Syncer | Docker | Casbin 策略同步 |
| **8082** | Swagger UI | Docker | API 文档可视化 |
