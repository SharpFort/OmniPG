...
> - v3.7（2026-08-07）— **前端对齐方案 §2.3/§2.4 审查修复（035 补丁）**：`get_user_menu` 再增 `component` 列（033 已回填 path→组件路径但原返回不下发——前端映射表 11 项硬编码，新菜单必改前端；补列后映射表降级兜底）；命名风格定稿 snake_case 透传（零转换层）；菜单模式本轮保持 frontend，下一轮优化 = 登录后拉取 get_user_menu+v_role_api_detail 入前端缓存、动态注册路由（backend 模式）
> - v3.6（2026-08-07）— **前端对齐方案 §2.2 API 层审查修复（035 补丁）**：① 删 `get_user_permissions`（uuid 变量接收 text nanoid 调用必炸——Casdoor 残留；输出均有替代：roles→get_current_user、权限码→v_role_api_detail、菜单→get_user_menu；casbin_rule 视图保留仅测试引用）；② **删 `cleanup_expired_tokens` 整链**（复核为死链：public 底层函数全库无定义 + 清理对象 sys_token_blacklist/sys_user_session 014 已删——D12 会话/吊销交 Logto 无可清理之物；wrapper/权限点 sys:session:cleanup/cron 任务 cleanup-expired-tokens 全删，v_system_stats_realtime.last_cleanup_time 改指 cleanup-old-audit-logs）；③ `get_dept_tree` 参数 uuid→text（017 text 化后未同步）；④ cron RPC 补 GRANT authenticated（021 孤例只授 web_anon）；⑤ `get_user_menu` 增 menu_type/perms/is_visible（前端 button 过滤）；⑥ rpc_update_config 源文件同步 029 门槛版
