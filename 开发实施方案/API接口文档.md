...
| 特性 | 实现 |
|:---|:---|
| 认证 | JWT + Refresh Token 轮转 + SSO 单设备登录 |
| 授权 | Casbin (RBAC + RESTful 路径匹配) |
| 多租户 | `tenant_id` 行级隔离 + RLS |
| 审计 | `sys_audit_log` 记录所有数据变更 |
| 安全 | Argon2id 密码哈希 + Token 黑名单 |
...
```
1. 客户端请求 → APISIX
2. APISIX 验证 JWT 签名（Casdoor JWKS）
3. APISIX Casbin 插件查询 casbin_rule 视图
...
| 003 | `20260707000003_create_casbin_view.sql` | DB Core | casbin_rule 视图（仅 p 规则，过滤软删除） |
