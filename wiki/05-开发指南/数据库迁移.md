...
| 配置 | `db/dbmate.toml`（`migrations_dir = "./migrations/platform"`） |
| 迁移目录 | `db/migrations/platform/`（当前仅 3 个基线文件：064/065/066） |
...
migrations_dir = "./migrations/platform"
```
...
- 目录：`db/migrations/platform/`（公共/系统域）。
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'xxx_name_not_null') THEN
        ALTER TABLE platform.xxx ALTER COLUMN name SET NOT NULL;
    END IF;
END $$;
...
历史 62 个迁移保存在 `git tag v0.1.0`（`git show v0.1.0 -- db/migrations/platform/` 可取回）。squash 的完整 SOP 见历史文档（18-迁移基线 Squash 指南，已归档）。
...
| 底层函数 / 触发器 / 视图 / RLS | `db/src/platform/`（functions/triggers/views/privileges） |
| 对外视图 / RPC（api_v1_platform.*） | `db/api_v1/<域>/`（views/rpc；目录按域组织 `_shared`/`platform`，当前活跃模块 platform；运行态仅暴露 api_v1_platform——新模块须同步 `apply-src.sh` 的 `API_MODULES` 与 compose `PGRST_DB_SCHEMAS`，postgrest.conf 参考文件已对齐（2026-08-19）） |
| GRANT | `db/api_v1/platform/privileges/zz_grant_all.sql` 集中管理 |

`scripts/apply-src.sh` 全量幂等重放顺序（**迁移最后执行**；模块清单 `MODULES="platform"`、`API_MODULES="_shared platform"`，新增域模块在此声明）：
...
| 命令 | 作用 |
| --- | --- |
| `make migrate` | 应用所有待执行迁移（`dbmate -d migrations/platform up`） |
...
> 参考：迁移文件的内容规范见 [编码规范.md](编码规范.md)，新增 API 流程见 [新增API流程.md](新增API流程.md)，目录职责见 [仓库结构.md](仓库结构.md)；部署链与冷启动细节见 [../03-部署指南/部署总览.md](../03-部署指南/部署总览.md) 与 [../03-部署指南/手动部署.md](../03-部署指南/手动部署.md)。