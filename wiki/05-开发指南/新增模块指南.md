...
> 定位：**为未来开发提供"新建一个业务域模块"的完整操作清单**——从新建目录、文件骨架，到需要在哪些配置中声明新模块，一次列全。区别于 [新增一个 API 的完整流程](新增API流程.md)（在现有模块内加一个视图/RPC），本文档回答的是**"新开一个业务域"**（如 billing、hr、crm）时所有需要触碰的位置。
>
...
| **A. 并入单 schema（谨慎使用）** | 业务对象并入 `platform` / `api_v1_platform`，按域分子目录组织 | 仅适合**极小型模块**或与现有 platform 强耦合的工具性功能 | 当前 `platform` 模块（权限管理） |

> ⚠️ **结论（2026-08-19 拍板）**：新建模块默认走**形态 B（独立 schema）**。`platform` 模块定位为**权限管理/系统基础**专用，不再承载新业务域；复杂项目应拆分为多个 schema 模块。
...
> 📚 模块划分经验：① 模块命名 = 业务域（billing/crm/hr…），不要用技术名；② 模块内聚合自洽（表+函数+视图+RLS 一起）；③ 跨模块只读查询优先走对方视图，写操作走对方 RPC；④ 通用/共享能力（权限、审计、通知）放 `platform` 模块。
...
db/
├── migrations/
│   └── <module>/                       # ① 本模块迁移目录（独立于 platform，见 §3.3 dbmate 方案）
...
│       ├── views/                      #    内部视图（非对外）
│       ├── templates/                  #    审计字段模板参考（按需，详见 [审计字段模板参考](审计字段参考.md)）
...
> 📌 形态 A（谨慎使用）时：目录同上但无独立迁移目录（并入 `migrations/platform/`），对象进 `platform` / `api_v1_platform`，仅在 `db/src/`、`db/api_v1/` 下按域分子目录。
...
| 9 | `db/migrations/<module>/` | 模块独立迁移目录（见 §3.3 dbmate 方案）；`deploy-db.sh` 需支持多目录循环 | ✅ 必改 | ❌（并入 `migrations/platform/`） |
...
| 12 | 文档 | 本页模块清单 + `仓库结构.md` 目录说明 + `架构总览.md` 模块划分表 | ✅ | ✅ |
...
```bash
MODULES="platform <module>"              # src 模块：platform（权限/基础）在前，新模块在后
API_MODULES="_shared platform <module>" # API 模块：_shared（跨模块共享）置首
```

> 说明：`_shared` 是跨模块共享 API 的占位目录（当前为空）；`platform` 是权限/系统管理域。新增模块追加在最后。
...
枚举写 `db/src/<module>/types/<name>.sql`（DO 块守卫、只增不删），因为 **bootstrap 阶段先应用 `src/*/types`**（`deploy-db.sh` 第一步），迁移依赖枚举必须先存在（依赖倒置陷阱，见 [数据库迁移.md](数据库迁移.md)）。
...
3. **部署链同步**：`scripts/deploy-db.sh` 与 `Makefile migrate` 目前是单目录（`dbmate -d migrations/platform up`），形态 B 落地时需改为上面的循环（这是形态 B 唯一需要改动的部署脚本）。

写表结构 + 数据（仅表/列/约束/索引/种子；**禁止**在迁移里写函数/视图/触发器/策略——17 号铁律，`apply-src.sh` 的 §6.3 扫描会拦截）。幂等三件套：`IF NOT EXISTS` / `DO` 块守卫 / `ON CONFLICT`。

> 💡 形态 A（谨慎）时：并入 `migrations/platform/`，用 3 位序号（当前 064/065/066，新文件从 067 起），无需改部署脚本。

### Step 4：写底层源码（db/src/<module>/）

- 函数：`db/src/<module>/functions/<name>.sql`，schema 为 `<module>`，幂等 `CREATE OR REPLACE`，SECURITY DEFINER 必须自校验 + `SET search_path = <module>, platform, pg_temp`（模板见 [新增API流程.md](新增API流程.md) Step 2）；
...
新建 `db/api_v1/<module>/privileges/zz_grant_all.sql`，按角色分层授予（参照 `db/api_v1/platform/privileges/zz_grant_all.sql` 的写法）：
...
在 `scripts/init-apisix-routes.sh` 第 4 段新增路由（参照现有 `api_v1_platform` 写法；优先级低于 `/rpc/webhook_logto` 的 95、高于 catch_all 的 10；proxy-rewrite 把 URL 前缀映射到 PostgREST schema）：
...
- [ ] 文档同步：本页模块清单、`仓库结构.md`、`架构总览.md` 模块划分表

---

## 6. 相关文档

- [新增一个 API 的完整流程](新增API流程.md) —— 在现有模块内加视图/RPC 的逐步清单
- [目录结构](仓库结构.md) —— db/ 目录总览
- [模块划分](../04-架构/模块划分.md) —— 业务域归属与权威方
- [迁移指南](数据库迁移.md) —— 迁移编号、幂等、依赖倒置
- [权限开发](权限开发指南.md) —— has_permission / RLS / 权限点
