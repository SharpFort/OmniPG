...
- 函数：`db/src/<module>/functions/<name>.sql`，schema 为 `<module>`，幂等 `CREATE OR REPLACE`，SECURITY DEFINER 必须自校验 + `SET search_path = <module>, public, pg_temp`（模板见 [新增API流程.md](新增API流程.md) Step 2）；
