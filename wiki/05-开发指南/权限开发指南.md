...
| 层 | 机制 | 管什么 | 代码位置 |
| --- | --- | --- | --- |
| ① 操作级 | `has_permission(code)` / `require_permission` / `require_super_admin` | 有没有资格做（写/管理/敏感操作） | `db/src/platform/functions/` |
| ② 数据级 | RLS 策略（20 条集中清单） | 能看到哪些行（租户/本人/部门范围） | `db/src/platform/privileges/rls_policies.sql` |
...
`menu_type` 枚举（`db/src/platform/types/menu_type.sql`）：`directory`（目录）/ `menu`（菜单）/ `button`（按钮=权限点）/ `link`（外链或 iframe）。

按钮行约束（基线 065 实测）：

- `menu_type='button'` → `api_code` 必填（权限码，命名空间 `platform:`，如 `platform:dept:create`）、`router`/`component` 必须 NULL（不污染 UI，CHECK `iam_menu_button_nav_null_check`）。
...
权限码命名空间：当前统一 `platform:`（055 D11 收敛，前端 v-perm 已同步）。业务模块预留 `content:` / `ugc:` 等。

菜单/权限点写入路径：运行时 `api_v1_platform.rpc_create_menu`（18 参，button 行校验 api_code/端点），或种子迁移（066 的 iam_menu 55 行先例）。

## 角色与数据范围 scope_type

`scope_type` 枚举（`db/src/platform/types/scope_type.sql`，059 转原生 ENUM）：
...
- `current_data_scope()`（`db/src/platform/functions/current_data_scope.sql`）：返回 `{scope_type, dept_ids}`；**多角色取最宽**：`all > dept_and_child > custom > self`。
...
`has_permission(p_code)`（`db/src/platform/functions/has_permission.sql`）判定逻辑：
...
```sql
-- 权限点档（写/管理 RPC 内一行调用）
PERFORM require_permission('platform:dept:create');   -- 不通过即 42501
...
集中清单：`db/src/platform/privileges/rls_policies.sql`（20 条策略，全部 `DROP POLICY IF EXISTS` + `CREATE POLICY` 幂等模板，挂最终表名）。常见模式：
...
以 `db/api_v1/platform/rpc/` 实际文件为准（44 个）：

| RPC | 权限码 | 作用 |
| --- | --- | --- |
| `rpc_set_role_menus(p_role_code, p_menu_ids uuid[])` | `platform:role-menu:bind` | 全量覆盖角色菜单绑定（授权=菜单树勾选） |
| `rpc_set_role_data_scope(p_role_code, p_scope_type, p_dept_ids)` | `platform:data-scope:bind` | 全量覆盖角色数据范围（custom 校验部门存在） |
| `rpc_get_role_data_scope(p_role_code)` | `platform:data-scope:bind` | 查询角色数据范围（默认 self） |
...
| `rpc_create_menu` / `rpc_update_menu` / `rpc_delete_menu` | `platform:menu:create/update/delete` | 菜单/按钮行 CRUD（button 行校验 api_code/端点/导航置空） |
| `rpc_get_menu_tree_admin()` | — | 管理端菜单树 |
| `rpc_create_department` 等 CRUD | `platform:dept:create/update/delete`、`platform:position:*`、`platform:dict:*` 等 | 各管理域写路径 |
| `rpc_search_users` / `rpc_list_tenant_members` | `platform:tenant-member:list`（部分） | 用户/成员查询 |
| `rpc_search_login_logs` | `platform:login-log:list` | 登录日志查询 |
| `rpc_import_csv` | `platform:import` | 通用导入（6 表白名单） |
| `update_config` | `platform:config:write` | 配置更新 |
| `webhook_logto(jsonb)` | —（web_anon 可调） | Logto webhook 入口 |

> 权限码全集以 `db/api_v1/platform/rpc/` 与 `db/src/platform/functions/` 中实际出现的 `platform:*` 字符串为准（当前 20+ 个码，grep 可复核）。新码先在 iam_menu 注册 button 行再使用。

## 示例：给角色挂菜单

```sql
-- 1) 拿到按钮/菜单 id（菜单树勾选结果）
SELECT id, menu_name, menu_type, api_code FROM api_v1_platform.iam_menu WHERE is_active;
...
1. **枚举加值**（值只增不删，改 `db/src/platform/types/scope_type.sql`，按 §8.3 模板追加守卫块）：
   ```sql
   DO $$
   BEGIN
       IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid
           ...
           ALTER TYPE platform.scope_type ADD VALUE 'dept_only';
       END IF;
   END $$;
   ```
...
6. **测试**：`db/tests/platform/` 增加 scope 分支断言；`make test-db` 通过。
7. 同步更新 [RPC清单.md](../06-API参考/RPC清单.md) 中 `rpc_get_role_data_scope`/`rpc_set_role_data_scope` 的取值说明。

> 注意：scope_type 是封闭枚举（只增不删），废弃值用 `z_deprecated_` 前缀 RENAME；改枚举不进函数签名（RPC 参数保持 text）。

## 开发检查清单

- [ ] 新写/管理 RPC：SECURITY DEFINER + `require_permission('platform:xxx:yyy')` + `SET search_path = platform, ext, pg_temp`？
...
> 参考：认证授权整体设计见 [../04-架构/认证授权设计.md](../04-架构/认证授权设计.md)，新增 API 流程见 [新增API流程.md](新增API流程.md)，编码规范见 [编码规范.md](编码规范.md)，RPC 参数明细见 [../06-API参考/RPC清单.md](../06-API参考/RPC清单.md)。
