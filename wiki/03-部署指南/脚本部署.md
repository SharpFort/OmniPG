...
| 步骤 | 动作 | 说明 |
| --- | --- | --- |
| [1/4] | bootstrap | `apply-src.sh $DB_URI --bootstrap`：init（扩展/schema/角色）+ `src/platform/types`（枚举）前置——迁移 059/060 引用 src 枚举，必须先建（依赖倒置修复） |
| [2/4] | dbmate up | `export DBMATE_DATABASE_URL=$DB_URI; dbmate up`（在 `db/` 下读取 dbmate.toml，migrations_dir=`./migrations/platform`） |
...
→ src/platform/types（枚举）→ src/platform 其余（functions/triggers/views/privileges）
→ api_v1/_shared + api_v1/platform 的 rpc → views → 其余（privileges/zz_grant_all.sql 排最后）
→ db/init → db/migrations/platform 重放（幂等性验证）
...
| `api_v1_platform` | `/api/v1/platform/*` | 50 | 重写 `^/api/v1/platform/(.*)` → `/$1`，JWT auth |
...
| [4] | 创建 7 条业务路由（logto_jwks / logto_proxy / webhook_logto / ensure_user / api_v1_platform / rpc_all / catch_all，均为 Admin API PUT，幂等） |
...
| 数据 | 来源 | 说明 |
| --- | --- | --- |
| 种子数据 | `db/migrations/platform/066_v010_seed_data.sql` | app_config(14)、dict_type(2)、dict_data(9)、iam_menu(55)，`ON CONFLICT (id) DO NOTHING` 幂等；iam_menu 按 parent_id 拓扑序 |
