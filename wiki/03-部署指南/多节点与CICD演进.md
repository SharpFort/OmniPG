...
| 2 | logto 库 | 多机后 **logto 库与业务库（app_db）及 platform 权限认证模块永远同机同集群** | ✅ 合理：消除跨机依赖；代价是同故障域（Logto 与 DB 同挂），当前规模可接受 | ✅ **已落地（2026-08-20）**：logto 库/用户已声明进 `infra/pigsty.yml` 与 `infra/pigsty.yml.tpl`（pg_databases/pg_users）；`LOGTO_DB_PASSWORD` 已加入 `.env.*` 与 `gateway/.env.example`；compose `DB_URL` 5433 不变 |
...
| 7 | 版本口径 | **以最终/当前版本为准**；不逐一修改其他文档中的版本号，在 Home 等主要位置提醒 | ✅ 同意：避免文档漂移；相关机制结论在 v4.4/v4.5 一致 | 已在 `wiki/首页.md` 顶部加版本口径提醒；其他文档版本号保持现状 |
