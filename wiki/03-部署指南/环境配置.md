...
| PGRST_PORT | 3100 | 3100 | 3100 | 3100 | PostgREST 宿主端口（3001/3002 已让给 Logto） |
...
| PGRST_PORT | 3100 | PostgREST 宿主端口 |
...
> ⚠️ 配置说明（2026-08-27 更新）：
> 1. compose 中 Logto `DB_URL` 走宿主 **5433**（运行态事实）；`infra/*.yml` 未定义 `logto` 库/用户，首次使用前需在宿主 PG 预创建（TODO）；
> 2. 根目录与 gateway 的 `PGRST_PORT` 已统一定为 **3100**，Swagger `API_URL` 默认值亦为 `http://localhost:${PGRST_PORT:-3100}/`；PostgREST 连库经宿主 pgBouncer **6432**，Swagger/OpenAPI 的 Base URL 由 `PGRST_OPENAPI_SERVER_PROXY_URI=http://localhost:9080` 显示为网关地址。
...
3. **PostgREST 端口**：compose 硬编码 `3100:3000`，`PGRST_PORT` 已在各 `.env*` / `gateway/.env.example` 统一为 3100；Swagger `API_URL` 为 3100（拉 spec），OpenAPI Base URL 显示 9080（网关唯一入口）。
