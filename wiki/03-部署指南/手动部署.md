...
> 前提：WSL2 Ubuntu（systemd）+ Docker Desktop（WSL2 集成）+ 仓库代码已就位。敏感项准备见 [环境配置.md](环境配置.md)。
...
#    => api_v1_platform / ext / net；USAGE 授权；pg_net 收紧（ext 为扩展宿主）

# 3) 枚举前置（bootstrap 子集；迁移 059/060 引用 src 枚举）
for f in db/src/platform/types/*.sql; do
  PGPASSWORD=dev_password_change_me psql -h 127.0.0.1 -U app_owner -d app_db \
...
## Step 4：dbmate 迁移（db/migrations/platform）
...
| PGRST_DB_SCHEMAS | `api_v1_platform` | `api_v1_platform`（2026-08-19 对齐） |
| PGRST_DB_EXTRA_SEARCH_PATH | `api_v1_platform,platform,ext` | `api_v1_platform, platform, ext`（2026-08-19 对齐） |
...
**暴露层（当前事实）**：运行态单 schema `api_v1_platform`（`db/api_v1/` 目录含 `_shared`（模块排序前缀）/ `platform`）；ext 为扩展宿主（pgcrypto/pgtap 迁入）；public 让给 Logto（40 号方案）。sales/inventory 测试模块已退役（063；相关声明与占位目录已于 2026-08-19 清理，按需重建时再补）。
...
> 🔒 2026-08-20 端口收敛：对外仅 9080；9180 仅内网管理（`allow_admin` 已收窄为私网段）；7085 仅绑本机回环 `127.0.0.1:7085`；9443 不再映射宿主。详见 [APISIX 端口收敛与安全访问](../08-运维/APISIX端口收敛.md)。
...
| api_v1_platform | `/api/v1/platform/*` | 50 | 重写 `^/api/v1/platform/(.*)` → `/$1`，jwt-auth（URL 前缀 /api/v1/platform/* 映射 api_v1_platform） |
...
容器事实：compose 中 Logto `DB_URL` 为 `postgres://logto:${LOGTO_DB_PASSWORD:-logto_dev_pass_2026}@host.docker.internal:5433/logto`（运行态走宿主 5433），entrypoint 会先 `npm run cli db seed -- --swe` 再执行 `logto-csp-patch.js` 后启动。⚠️ `logto` 库/用户未定义在 `infra/*.yml` 中，首次部署前需在宿主 PG 预创建（见 [环境配置.md](环境配置.md)）。

**对应脚本**：`scripts/phase2/init-logto.py`（不在 deploy-all 链中）；webhook 链路见 [06-API参考/Logto Webhook](../06-API参考/LogtoWebhook接入.md)。
...
验收重点：PostgREST OpenAPI 完整（>2000 字符）、APISIX 路由 8 条、Logto OIDC discovery 可访问、`/api/v1/platform/*` 无 token 401 / 带 Logto token 200、镜像表只读。
...
- [脚本部署（一键方案）](脚本部署.md)
- [升级与回滚](升级与回滚.md)
- [05-开发指南/数据库迁移](../05-开发指南/数据库迁移.md)
- [06-API参考/PostgREST](../06-API参考/PostgREST使用指南.md)
- [06-API参考/网关路由](../06-API参考/网关路由.md)
- [07-测试/验证脚本](../07-测试/冒烟验证脚本.md)

> 参考：本页步骤与 `scripts/` 目录、`infra/`、`gateway/docker-compose.yml`、`db/init/`、`db/migrations/platform/` 当前代码一一对应；历史文档（17/19/20 号）中的 standalone 模式与 Casdoor 内容已过时。