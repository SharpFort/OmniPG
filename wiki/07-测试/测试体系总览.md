...
| 层级 | 载体 | 位置 | 入口命令 | 覆盖目标 |
| --- | --- | --- | --- | --- |
| 数据库单元测试 | pgTAP | `db/tests/platform/*.sql`（5 个文件，计划断言合计 107 条） | `make test-db` | 表/列/索引/外键/函数/触发器/RLS 的存在性与基础行为 |
...
- `verify-stack.sh`：8 项组件健康检查（依赖预检、网络链路、pgbouncer、PostgREST、APISIX Status、Dashboard、路由 7 条、Logto OIDC、无 docker PG 残留；2026-08-19 已按 Logto 架构更新，见 [冒烟验证脚本](冒烟验证脚本.md)）。
...
5. **验证脚本过时项**：`verify-stack.sh` 与 `deploy-gateway.yml` 仍引用 Casdoor 时代脚本/组件（见 [冒烟验证脚本](冒烟验证脚本.md) 的 TODO）。
...
> 参考：[pgTAP 测试指南](pgTAP测试指南.md) · [E2E 集成测试](E2E集成测试.md) · [冒烟验证脚本](冒烟验证脚本.md) · [数据库迁移](../05-开发指南/数据库迁移.md) · [认证与授权设计](../04-架构/认证授权设计.md) · [Logto Webhook 接入](../06-API参考/LogtoWebhook接入.md)