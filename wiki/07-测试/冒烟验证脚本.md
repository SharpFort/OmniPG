...
| 6 | 路由清单 | Admin API 路由数 = 7（logto_jwks/logto_proxy/webhook_logto/ensure_user/api_v1_platform/rpc_all/catch_all） | 现行 |
...
验证“从空库到全量结构”可复现（v0.1.0 squash 基线，`db/migrations/platform` 仅 064/065/066：064 镜像表 6 张 + 065 业务表 18 张 = 24 张表；066 种子 app_config 14 / dict_type 2 / dict_data 9 / iam_menu 55；历史 62 个迁移在 git tag v0.1.0）。默认 scratch 库 `app_db_verify`（保留复用，每次 DROP+重建），参照库 `REF_DB`（默认 `app_db`）做结构比对。
...
| 3 | bootstrap 子集 | `02-schemas.sql` + `db/src/platform/types/*.sql`（枚举前置） |
...
| §4 | 死端点判定（对照 `api_v1_platform` 实际暴露的视图/RPC） |
...
> 参考：[测试体系总览](测试体系总览.md) · [E2E 集成测试](E2E集成测试.md) · [部署指南总览](../03-部署指南/部署总览.md) · [数据库迁移](../05-开发指南/数据库迁移.md) · [Logto Webhook 接入](../06-API参考/LogtoWebhook接入.md)