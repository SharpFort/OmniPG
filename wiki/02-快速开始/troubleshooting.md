...
- 镜像拉取失败：检查网络/镜像源（见 [前置条件](prerequisites.md)）。
...
- 迁移目录 `db/migrations/public`（当前为 squash 基线 064/065/066）；`make migrate-status` 查看状态。
...
> 参考：[前置条件](prerequisites.md) · [一键搭建本地开发环境](one-click-dev.md) · [第一个 API 调用](first-api-call.md) · [部署指南总览](../03-部署指南/deployment-overview.md)
