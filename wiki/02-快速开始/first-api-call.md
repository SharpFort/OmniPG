...
- ⚠️ `gateway/.env.example` 中 `PGRST_PORT=3001` 与 compose 的 3100 映射不一致（Swagger 的 API_URL 也引用该变量）；以 3100 为准，详见 [常见问题排查](troubleshooting.md)。
...
> RPC 清单与参数见 [RPC 参考](../06-API参考/rpc-reference.md) 与 Swagger（http://localhost:8082）。RPC 形参名必须与 SQL 函数签名一致（PostgREST 按参数名匹配，错误会 400/404）。
...
> 排障细节见 [常见问题排查](troubleshooting.md)。

---

> 参考：[网关路由](../06-API参考/gateway-routing.md) · [PostgREST 使用指南](../06-API参考/postgrest.md) · [RPC 参考](../06-API参考/rpc-reference.md) · [认证与授权设计](../04-架构/auth-design.md) · [E2E 集成测试](../07-测试/e2e-tests.md)