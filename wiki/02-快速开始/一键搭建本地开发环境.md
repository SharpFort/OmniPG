...
> 数据库（Pigsty 宿主）须已就绪；未安装基础设施请看 [前置条件](前置条件.md)。各步骤失败的表现与排查见 [常见问题排查](常见问题排查.md)。
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| DB_HOST / DB_PORT / DB_USER / DB_NAME | host.docker.internal / 5432 / app_owner / app_db | 网关侧参考值（PostgREST 宿主端口 3100；连库经 host.docker.internal:6432 走 pgBouncer） |
| AUTHENTICATOR_PASSWORD | authenticator_dev_pass | PostgREST 连库密码，必须与 Pigsty `pg_users` / `userlist.txt` 一致（pgBouncer 重载后生效） |
...
| JWKS_JSON | HS256 dev key（占位） | 开发环境为 HS256 对称密钥；staging/production 指向 Logto JWKS 公钥（RS256 口径，见 [第一个 API 调用](第一个API调用.md)；compose 与 .env.staging/.production 注释写 ES384，口径不一致，需以 Logto 实际配置核实） |
| PGRST_PORT | 3100 | 与 compose 中 PostgREST 实际映射 `3100:3000` 一致（3001/3002 已让给 Logto） |
...
| postgrest | app-postgrest | 3100 → 3000 | 暴露 `api_v1_platform` schema；连接 pgbouncer 6432 |
| swagger-ui | app-swagger | 8082 → 8080 | 浏览器端拉取 PostgREST OpenAPI |
| logto | app-logto | 3001 / 3002 | 首次启动执行 `db seed -- --swe` + CSP 补丁后 `npm start` |

> 🔒 2026-08-20 端口收敛：对外仅 9080；9180 仅内网管理（allow_admin 收窄私网段）；7085 仅绑本机回环；9443 不再映射宿主（预留）。详见 [APISIX 端口收敛与安全访问](../08-运维/APISIX端口收敛.md)。

关键环境变量（compose 内，代码事实）：

- PostgREST（运行态以 compose 环境变量为权威）：`PGRST_DB_URI=postgres://authenticator:***@host.docker.internal:6432/app_db`；`PGRST_DB_SCHEMAS=api_v1_platform`（单 schema）；`PGRST_DB_EXTRA_SEARCH_PATH=api_v1_platform,platform,ext`；`PGRST_JWT_SECRET=${JWKS_JSON}`；`PGRST_JWT_ROLE_CLAIM_KEY=.pg_role`（DB 角色取自 JWT 的 `pg_role` claim）；`PGRST_MAX_ROWS=1000`；`PGRST_DB_PRE_REQUEST` 为空（不启用黑名单）。`gateway/postgrest/postgrest.conf` 仅为参考文件（2026-08-19 已与运行态对齐：单 schema、.pg_role），不挂载进容器。
...
4. `PUT` 业务路由 7 条：`logto_jwks`、`logto_proxy`（`/logto/*`）、`webhook_logto`（`/rpc/webhook_logto`，HMAC 验签）、`ensure_user`（`/rpc/ensure_user`）、`api_v1_platform`（`/api/v1/platform/*`）、`rpc_all`（`/rpc/*`）、`catch_all`（`/*`）。
5. `PUT /apisix/admin/global_rules/1`：全局 CORS（含 `logto-signature-sha-256` header）。

> ✅ 2026-08-19：部署链（`make dev`）已切换为 Logto 版 `scripts/init-apisix-routes.sh`（清理旧路由、Logto JWKS RS256、`/logto/*` 同源代理、`/rpc/webhook_logto` HMAC 验签、`/rpc/ensure_user`、`/api/v1/platform/*` 重写为 `/$1`）；Casdoor 时代 `setup_apisix.sh` 已删除。
...
- 迁移目录 `db/migrations/platform`；当前为 squash 基线三件套（共 24 张表）：
...
- 登录并调用第一个接口 → [第一个 API 调用](第一个API调用.md)
- 各步骤失败排查 → [常见问题排查](常见问题排查.md)

---

> 参考：[前置条件](前置条件.md) · [第一个 API 调用](第一个API调用.md) · [常见问题排查](常见问题排查.md) · [脚本部署](../03-部署指南/脚本部署.md) · [数据库迁移](../05-开发指南/数据库迁移.md)