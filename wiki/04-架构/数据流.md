...
| JIT 建档 | 登录回调调 `POST /rpc/ensure_user`（目标路由 init-apisix-routes.sh：jwt-auth key_claim_name=sub）→ `api_v1_platform.ensure_user()`：users/user_profile 仅缺失补建（N7 不覆盖 webhook 权威值）；user_role 按 global_roles / org_roles 分段精确对齐（049 D5/D6） |
...
Logto（事件源）              APISIX               PostgREST                PostgreSQL
  ...
  │ Role.* / PostSignIn
  ├── POST /rpc/webhook_logto ──▶ HMAC 验签（目标路由） ──▶ api_v1_platform.webhook_logto(payload)
...
业务请求进入 PostgreSQL
  ...
  ├─ 写/管理类（DEFINER RPC）：
  │    require_permission('platform:xxx') / has_permission('platform:xxx')
...
关键函数（db/src/platform/functions/）：
...
管理端授权写路径：`rpc_set_role_menus(p_role_code, p_menu_ids)`（权限点 platform:role-menu:bind；全量覆盖 iam_role_menu + log_operate 审计）→ `rpc_set_role_data_scope(p_role_code, p_scope_type, p_dept_ids)`（platform:data-scope:bind；custom 校验部门存在性）。
...
审计触发器挂载（db/src/platform/triggers/trg_audit_*.sql，9 张业务表；镜像表不挂）：
...
| 审计详情视图 | api_v1_platform.v_audit_log_detail / audit_log 视图 | authenticated SELECT + RLS |
| 登录日志搜索 | rpc_search_login_logs（has_permission('platform:login-log:list') + 租户成员过滤） | 权限点 |
| 登录日志视图 | v_login_log（LATERAL geo_locate 实时地理 join） | authenticated SELECT + RLS（login_log_read_policy：超管/本租户/本人） |

IP 归属链路（db/src/platform/functions/geo_locate.sql + ip2region.sql）：`sync_login_log_write` 落库时用 `ip2region(ip)`（ip_region_v4，仅 IPv4）写 region 快照；查询时 `geo_locate(ip)` 实时解析（ip2region 优先 → GeoLite2 ip_geolite2_city 兜底，含 IPv6/经纬度/时区）。
...
Logto 投递 ──▶ INSERT webhook_event_log（result=received）
             ...
             超管重放 rpc_replay_webhook_event(p_event_id)
                 └─▶ 取 payload 重新调用 api_v1_platform.webhook_logto
...
> 参考：登录与 token 生命周期详见 [认证与授权设计](./认证授权设计.md)；webhook 事件表与重放见 [Logto webhook](../06-API参考/LogtoWebhook接入.md)；RPC 签名见 [RPC 参考](../06-API参考/RPC清单.md)；审计读写规范见 [审计日志](../08-运维/审计与日志.md)。