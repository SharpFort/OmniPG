...
- 保留 `casbin_rule` 视图（db/src/platform/views/casbin_rule.sql）为双段投影：API 段 = iam_role_menu→button 行（v1=api_url, v2=api_method）+ 菜单段（v1=router, v2='menu'），仅测试/兼容消费（2026-08-20 已随兼容视图清理移除，不再借鉴 Casbin 模型）。
...
| B：PostgREST | 暴露对外层 schema（api_v1_platform 等，多 schema 声明）；视图=REST 表、函数=/rpc；RLS 为安全边界 | ✅ 采纳 |
| C：pg_graphql 替代 | GraphQL 面 | 作为补充扩展安装（Pigsty 管理），主面仍 REST |

### 决策

- PostgREST v14（gateway/docker-compose.yml postgrest 服务）暴露对外层 schema：docker-compose env 运行时为 `api_v1_platform`（env 覆盖 conf），postgrest.conf 参考文件已对齐单 schema api_v1_platform（2026-08-19），extra search path = api_v1_platform, platform, ext。
...
- 扩展权威清单：infra/pigsty.yml（唯一 inventory，2026-08-19 方案 A；pg_extensions 装包 + pg_databases[].extensions 库内启用），说明文档见 wiki/01-项目简介/扩展/；db/init/01-extensions.sql 已于 2026-08-19 移除。
...
- **17 号铁律**：迁移文件只承载表结构 + 数据（幂等 IF NOT EXISTS / DO 块）；函数/视图/触发器/类型/RLS 归 db/src/platform/ 与 db/api_v1/platform/，apply-src 全量幂等重放（含迁移目录代码对象扫描）。
...
| B：单库多 schema | platform（核心+授权+审计）→ api_v1_platform（暴露层）→ 扩展 schema | ✅ 采纳 |
| C：单 schema 按目录分模块 | 无物理权限边界 | ❌ 权限粒度不够 |

### 决策

- `platform`：核心业务（镜像表 + 自主表 + 授权 + 审计 + 日志）；`api_v1_platform`：对外 API 暴露层（视图 + RPC，只放投影不放物理表）；net/cron：扩展宿主 schema。
- 027 迁移把 api_v1_sys 收敛为 api_v1_platform（视图名 = 底层表名）；对外暴露层为单 schema api_v1_platform（postgrest.conf 已于 2026-08-19 对齐；api_v1_sys 遗留 schema 已于 2026-08-20 移除）；063 迁移退役 inventory/sales 测试模块（路由已移除、postgrest.conf 声明与占位目录已清理、按需重建时再补）。
- dbmate 迁移按 schema 分目录（db/migrations/platform/）；PostgREST 暴露层：docker-compose env 运行时为 api_v1_platform；postgrest.conf 参考文件已对齐（2026-08-19）。

### 影响与代价

- 模块边界 = PG 权限边界（GRANT USAGE + 表级授权）；新模块需同步 postgrest 配置与 apply-src 模块声明。
- 跨模块依赖方向受 apply-src 模块顺序约束（platform 先于 api_v1；api_v1 内 privileges 排最后）。
...
- dbmate（db/dbmate.toml + Makefile migrate/migrate-rollback/migrate-status）管理 db/migrations/platform/。
...
> 参考：选型对比细节见 [项目总纲](../01-项目简介/项目概览.md) 与 [技术栈全景](../01-项目简介/技术栈.md)；迁移规范见 [数据库迁移](../05-开发指南/数据库迁移.md)；权限模型见 [认证与授权设计](./认证授权设计.md) 与 [权限开发指南](../05-开发指南/权限开发指南.md)。