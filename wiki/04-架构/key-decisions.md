...
> 本文以 ADR（Architecture Decision Record）形式记录关键选型：每篇按「背景 → 可选方案 → 决策 → 影响与代价」展开。事实依据：docs/开发实施方案/05 号（Logto 认证与权限架构）、00 号（项目总纲）、05.4/05.5、16 号（菜单单表化）、36 号（迁移基线）与当前代码。
