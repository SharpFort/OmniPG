...
│  · 路由：/api/v1/platform/* 去前缀→schema、/logto/*、/rpc/*  │         │
│  · 全局 CORS；对外 9080(HTTP)，管理 9180(仅内网)          │         │
└──────────────┬───────────────────────────────────────────┘         │
               ▼                                                      ▼
┌──────────────────────────────────────────────────────────┐  ┌───────────────────────┐
│ PostgREST（容器 3000 / 宿主 3100）                        │  │ /rpc/webhook_logto     │
│  · 暴露层 schema：api_v1_platform 等（conf 多 schema 声明） │◀─┘（webhook 接收 RPC）     │
...
│  · platform：业务表/RPC/视图/触发器/RLS（镜像表 + 自主表）    │
...
| postgrest | postgrest/postgrest:v14.15 | 3000 | 3100 | REST 引擎：api_v1_platform schema 自动映射 + /rpc/* 函数端点 |
...
3. **网关鉴权**：客户端携带 Authorization: Bearer <JWT> 请求 APISIX → jwt-auth 插件用 Logto JWKS 验签 → 命中路由（如 /api/v1/platform/* 经 proxy-rewrite 去掉前缀，映射到 api_v1_platform 等 schema 对象）→ 全局 CORS 处理 → 转发 PostgREST。
...
6. **webhook 旁路**：Logto 事件（用户/组织/成员/角色/登录）异步 POST 到 http://host.docker.internal:9080/rpc/webhook_logto（经网关进入 PostgREST），由 api_v1_platform.webhook_logto 分发到 sync_* 函数写镜像表。
...
| api_v1_platform | /api/v1/platform/* | app-postgrest:3000 | proxy-rewrite + jwt-auth | 50 | 去掉前缀 ^/api/v1/platform/(.*) → /$1，映射 api_v1_platform 等 schema 对象 |
...
> api_v1_sales / api_v1_inventory 路由已随 063 退役移除（init-apisix-routes.sh 注释：测试模块退役，后续按需重建）；`/api/v1/platform/*` 是 API 暴露层 URL 前缀（与 schema api_v1_platform 同名；原 `/api/v1/sys/*` 历史前缀已于 2026-08-20 迁移对齐），表名本身无 sys_ 前缀。
...
| API 层 | PostgREST | 把对外暴露层 schema（单 api_v1_platform；conf 参考文件 2026-08-19 已对齐）暴露为 REST；/rpc/* 函数端点；注入 request.jwt.claims；OpenAPI 生成 | gateway/docker-compose.yml（postgrest 环境变量，运行态权威）、gateway/postgrest/postgrest.conf（参考文件） |
...
| PGRST_DB_SCHEMAS | api_v1_platform（compose env，运行时生效） | 运行时以 env 为准；postgrest.conf 参考文件已对齐单 schema（2026-08-19） |
| PGRST_DB_EXTRA_SEARCH_PATH | api_v1_platform,platform,ext | 函数解析搜索路径 |
...
> 配置来源说明：**运行态以 gateway/docker-compose.yml 为权威**（单 schema api_v1_platform、.pg_role、extra search path = api_v1_platform,platform,ext、max-rows 1000、pre-request 已清空、宿主 3100）；postgrest.conf 为参考文件（2026-08-19 已与运行态对齐：单 schema、.pg_role、无 pre-request；jwt-secret="$(JWKS_JSON)" 由 gateway/.env 注入），以 compose 为运行态权威。

## 模块划分总览（按 schema）

| Schema | 职责 | 目录 |
| --- | --- | --- |
| platform | 核心业务：镜像表（users/tenants/role/…）、自主表（iam_menu/iam_role_menu/…）、全部业务函数/触发器/视图/RLS、审计与日志 | db/src/platform/、db/migrations/platform/ |
| public | Logto 认证/授权占用（40 号方案：业务迁出后整体让给 Logto） | gateway/docker-compose.yml（Logto 直连 app_db） |
| ext | 扩展宿主（40 号方案：pgcrypto/pgtap 迁出 public 至此） | 扩展管理（db/init/02-schemas.sql） |
| api_v1_platform | 对外暴露层（现行）：视图投影（视图名 = 底层表名）与 RPC 包装（api_v1_platform.*） | db/api_v1/platform/ |
...
| 扩展（非 schema） | ext 为扩展宿主：pgcrypto/pgtap 已迁入 ext，pg_net 宿主 net，pg_cron 宿主 cron；ip2region/GeoLite2 离线表在 platform；说明文档见 wiki/01-项目简介/扩展/（权威 = infra/pigsty.yml） | wiki/01-项目简介/扩展/、infra/pigsty.yml |

> schema 现实（以 db/init/02-schemas.sql 为准）：platform、api_v1_platform、net（pg_net 宿主）、ext（扩展宿主：pgcrypto/pgtap 迁入）；public 让给 Logto（40 号方案）——不存在 api_v1_sales / api_v1_inventory schema（063 已退役；postgrest.conf 声明与占位目录已于 2026-08-19 清理，按需重建时再补）。运行态暴露层为单 schema api_v1_platform。db/api_v1/ 目录含 _shared（空，apply-src API 模块排序前缀）、platform（实际内容）。原兼容视图 platform.sys_user / platform.casbin_rule 已于 2026-08-20 移除（不再借鉴 Casbin 权限模型），用户数据直接查 platform.users + platform.user_profile。db/api_v1/platform/privileges/zz_grant_all.sql 与 db/src/platform/privileges/rls_policies.sql 是授权/策略的集中清单。详见 [模块划分](./模块划分.md)。
...
- **RLS 集中清单**：db/src/platform/privileges/rls_policies.sql 共 20 个策略（租户隔离 RESTRICTIVE、全局共享只读、超管豁免等），全部读取 PostgREST 注入的 request.jwt.claims，零查询。
- **SECURITY DEFINER + search_path 锁定**：写/管理 RPC 与 helper 一律 SECURITY DEFINER SET search_path = platform, ext, pg_temp，函数内自校验（has_permission / require_super_admin）——DEFINER 会绕过 RLS，必须函数内兜底。
- **17 号铁律（代码对象归位）**：迁移文件只承载表结构 + 数据（幂等 IF NOT EXISTS / DO 块）；函数/视图/触发器/类型/RLS 归 db/src/platform/ 与 db/api_v1/platform/；scripts/apply-src.sh 全量幂等重放（含 §6.3 迁移目录代码对象扫描，命中即失败）。
...
> 参考：分层/RLS/防御思想源自 [项目总纲](../01-项目简介/项目概览.md)；技术栈细节见 [技术栈全景](../01-项目简介/技术栈.md)；网关路由见 [网关路由](../06-API参考/网关路由.md)；PostgREST 配置见 [PostgREST 配置](../06-API参考/PostgREST使用指南.md)；模块归属见 [模块划分](./模块划分.md)。