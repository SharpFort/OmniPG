...
| 3100 | PostgREST | 3000 | 调试/OpenAPI 直连；**业务必须走 9080**（OpenAPI Base URL 亦为 9080） |
| 8082 | Swagger UI | 8080 | `API_URL` 指向 PostgREST（${PGRST_PORT}=3100，拉 spec）；Base URL 显示 9080（网关入口） |
...
| 6432 | pgBouncer（宿主 Pigsty） | — | PostgREST 经 `host.docker.internal:6432` 连接；Logto 走宿主 PG `5433` |
