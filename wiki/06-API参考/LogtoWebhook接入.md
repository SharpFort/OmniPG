...
| 验签 | 请求头 `logto-signature-sha-256` = HMAC-SHA256(signingKey, **rawBody**) 的 hex；由 APISIX `serverless-pre-function` Lua 校验（见 [网关路由](./网关路由.md)） |
...
管理端 RPC（均 require_super_admin，见 [RPC 清单](./RPC清单.md)）：
...
> 参考：[RPC 清单](./RPC清单.md) · [网关路由](./网关路由.md) · [PostgREST 使用指南](./PostgREST使用指南.md) · [认证与授权设计](../04-架构/认证授权设计.md) · [数据流](../04-架构/数据流.md) · [安全](../08-运维/安全设计.md)
