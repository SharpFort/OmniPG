...
- 对外暴露：`db/api_v1/` 含 `_shared`（空） / `platform`（当前有效暴露代码），`platform` 下 **44 个 RPC**、**29 个视图**（v0.1.0 基线，文件数核实）；
- 库内逻辑：`db/src/platform/functions/` 37 个函数、10 个触发器、RLS 策略全表启用（`db/src/platform/privileges/rls_policies.sql`）。
...
| 2026-08-05 ~ 08-12（实现收敛） | webhook 同步全部在库内（`rpc_webhook_logto` + `sync_*`）；`has_permission`（023）；`sys_` 前缀移除；Schema `api_v1_sys` → **`api_v1_platform`**（027）；菜单权限单表化（055：`iam_api`/`iam_role_api` 删除，权限点内嵌 `iam_menu` 按钮行，仅剩 `iam_role_menu` 绑定表） | 授权数据收敛为"镜像（Logto 权威）+ 自主（PG 权威）"两类 |
...
**吸收了什么**：casbin 的 RBAC 思路保留在数据库内——角色→权限绑定表（`iam_role_menu` → `iam_menu` 按钮行端点）继续承载 RBAC 语义（原 `casbin_rule` 兼容投影视图已于 2026-08-20 移除）；RLS 数据隔离是独立于网关的第二道防线。`platform.sys_user` 兼容视图亦已移除，用户数据直接查 `platform.users`（Logto 镜像）+ `platform.user_profile`（业务档案）。
...
- **扩展选型**以历史扩展选型审查（23/24 号，已归档）的决策为纲；**权威清单 = `infra/pigsty.yml`**（`pg_extensions` 节点级装包 + `pg_databases[].extensions` 库内启用），逐扩展说明见 [扩展/](扩展/)：
  ...
  - 运行态查询：`pg_available_extensions`（节点已装包）/ `pg_extension`（当前库已启用），各扩展说明见 [扩展/](扩展/)。
...
| Schema 收敛 `api_v1_platform`、菜单权限单表化（055） | 2026-08-05 ~ 08-12 | ✅ |
...
> 参考：[技术栈全景](技术栈.md) · [架构总览](../04-架构/架构总览.md) · [认证与授权设计](../04-架构/认证授权设计.md) · [部署指南总览](../03-部署指南/部署总览.md) · 设计决策过程见历史文档（00 号项目总纲、05 号 Logto 方案，已归档）；正文以当前代码为准。