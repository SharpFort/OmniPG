...
| **PostgreSQL** 18 | 唯一数据与逻辑核心：表、视图、RPC、触发器、RLS、扩展 | `db/init/`、`db/migrations/platform/`、`db/src/platform/`、`db/api_v1/platform/` |
| **pgBouncer** | 连接池（PostgREST 经 6432 接入；scram-sha-256） | `infra/pgbouncer.ini`、`infra/userlist.txt` |
| **PostgREST** v14.15 | REST API 层：自动映射 `api_v1_platform` 的表视图/RPC，注入 JWT claims，生成 OpenAPI | `gateway/docker-compose.yml`（postgrest 服务环境变量）；参考配置 `gateway/postgrest/postgrest.conf` |
...
| **dbmate** | 数据库迁移（`migrations/platform`） | `db/dbmate.toml`、`Makefile`（`make migrate / migrate-rollback / migrate-status`） |
...
| Schema | 内容 | 说明 |
| --- | --- | --- |
| `platform` | 核心业务表 + 全部函数/触发器/视图/RLS 策略 | 业务逻辑唯一真相源；镜像表（users/tenants/role/user_tenants/user_role 等）只读投影 |
| `api_v1_platform` | 系统管理 API 暴露层：视图 + RPC（`api_v1_platform.*`），URL 前缀 `/api/v1/platform/*` 映射至此 | 027 定稿名；**当前 compose 运行态实际暴露**（`PGRST_DB_SCHEMAS=api_v1_platform`） |
| ~~`api_v1_sales` / `api_v1_inventory`~~ | ~~销售 / 库存测试域暴露层~~ | 2026-08-15 退役；2026-08-19 已从 `postgrest.conf` 移除，占位目录已清理——运行态仅 `api_v1_platform` |
| `net` | pg_net 扩展宿主 schema | 权限收紧：`authenticated` 无 EXECUTE/USAGE，HTTP 调用一律经 SECURITY DEFINER 封装函数 |

代码目录：`db/migrations/platform`（dbmate 迁移）、`db/src/platform`（函数/触发器/类型/视图/RLS）、`db/api_v1/`（`_shared`（空） / `platform`（当前唯一有效暴露代码，44 RPC + 29 视图））、`db/init`（扩展 + schema 引导）、`db/tests`（pgTAP）。

**兼容视图已移除**（2026-08-20）：`platform.sys_user` 与 `platform.casbin_rule` 兼容视图已删除——用户数据直接查 `platform.users`（Logto 镜像）+ `platform.user_profile`（业务档案，对外经 `api_v1_platform.users`），权限模型不再借鉴 Casbin（授权以 `iam_role_menu → iam_menu` 为准）。

## PostgreSQL 扩展清单（权威 = `infra/pigsty.yml`，唯一 inventory）

> **声明即权威**：`pg_extensions`（节点级装包）、`pg_databases[].extensions`（库级启用）；逐扩展说明见 [扩展/](扩展/)，运行态事实以 `pg_available_extensions`（已装包）/ `pg_extension`（当前库已启用）查询为准。

### 已启用（库内 CREATE EXTENSION）

| 扩展 | 用途 | 说明页 |
| --- | --- | --- |
| `pgcrypto` | 辅助加密（sha256/HMAC/gen_random_uuid，非密码场景） | [加密辅助.md](扩展/加密辅助.md) |
| `pg_net` | 异步 HTTP（webhook 回调等），宿主 net | [异步HTTP.md](扩展/异步HTTP.md) |
| `pgtap` | pgTAP 单元测试框架 | [pgTAP单元测试.md](扩展/pgTAP单元测试.md) |
| `pg_cron` | 定时任务（`cron.job`/`cron.job_run_details`，经 `rpc_list_cron_jobs` 等只读 RPC 暴露） | [定时任务.md](扩展/定时任务.md) |
| `pg_graphql` | PG 原生 GraphQL（预留，暂未用于业务 API） | [GraphQL支持.md](扩展/GraphQL支持.md) |

### 已拍板待启用（24 号文档批次，yml 已列，即将启用）

| 扩展 | 用途 | 说明页 |
| --- | --- | --- |
| `safeupdate` | 防误删/误更新（仅装包 + preload，Load=是 Create=否，无 CREATE EXTENSION） | [安全更新.md](扩展/安全更新.md) |
| `plpgsql_check` | PL/pgSQL 静态检查 | [PLpgSQL静态检查.md](扩展/PLpgSQL静态检查.md) |
| `pg_jsonschema` | JSONB JSON Schema 校验（update_config 校验） | [JSONSchema校验.md](扩展/JSONSchema校验.md) |
| `omni_csv` | CSV 解析/加载（export/import_csv 重写） | [CSV解析.md](扩展/CSV解析.md) |
| `pgmemento` | 审计与时间旅行（P1 试点） | [审计与时间旅行.md](扩展/审计与时间旅行.md) |
| `pg_mockable` | 函数 mock（pgTAP 配套单测） | [函数模拟.md](扩展/函数模拟.md) |
| `jsquery` | JSONB 查询语言 | [JSONB查询语言.md](扩展/JSONB查询语言.md) |
| `index_advisor` | 索引建议工具 | [索引建议.md](扩展/索引建议.md) |
| `pg_repack` | 在线表膨胀治理 | [表膨胀治理.md](扩展/表膨胀治理.md) |
...
- **职责**：把暴露层 schema 的视图/RPC 自动映射为 REST 接口。**运行态以 `gateway/docker-compose.yml` 为权威**：`PGRST_DB_SCHEMAS=api_v1_platform`（单 schema，仅系统管理域）、`PGRST_DB_EXTRA_SEARCH_PATH=api_v1_platform,platform,ext`、`PGRST_MAX_ROWS=1000`、`PGRST_DB_PRE_REQUEST` 已清空。`gateway/postgrest/postgrest.conf` 为**参考文件**（2026-08-19 已与运行态对齐：单 schema `api_v1_platform`、`.pg_role`；引用时仍以 compose 为运行态权威）。OpenAPI spec 由 Swagger UI（8082）消费，宿主端口 3100（容器内 3000）。
...
- **目标路由集**（Logto 版，`scripts/init-apisix-routes.sh`，7 条）：`/api/v1/platform/*` 经 proxy-rewrite（regex `^/api/v1/platform/(.*) → /$1`）重写至 PostgREST 的 api_v1_platform 暴露层：
...
| 50 | `/api/v1/platform/*` | JWT 保护：业务 API（api_v1_platform） |
...
| 文件 | 定位 | 现状 |
| --- | --- | --- |
| `scripts/init-apisix-routes.sh` | **唯一路由初始化脚本**（Logto 版：logto_jwks → app-logto:3001、logto_proxy `/logto/*`、webhook_logto `/rpc/webhook_logto`（HMAC 验签）、ensure_user `/rpc/ensure_user`、api_v1_platform `/api/v1/platform/*`、rpc_all `/rpc/*`、catch_all `/*`；开头幂等清理 Casdoor 时代残留路由） | ✅ 部署链唯一入口：`Makefile dev`、`scripts/deploy-all.sh`、`.github/workflows/deploy-gateway.yml` 均调用它；依赖 `gateway/.env` 的 `APISIX_ADMIN_KEY` 与 `LOGTO_WEBHOOK_SIGNING_KEY`（缺失 fail-closed） |
...
| 工具 | 用法 | 说明 |
| --- | --- | --- |
| dbmate | `make migrate` / `make migrate-rollback` / `make migrate-status` | 迁移目录 `db/migrations/platform`，连接串来自 `gateway/.env` 的 `DB_PASSWORD` |
...
- 端口速查（开发环境，2026-08-20 收敛）：APISIX 9080（对外）/9180（Admin+Dashboard，仅内网管理）/7085（Status，仅本机回环；9443 预留未映射宿主） · PostgREST 3100 · Logto 3001(core)/3002(console) · Swagger 8082 · PostgreSQL 5432 · pgBouncer 6432 · Redis 6379 · Logto 业务库 5433（宿主 Pigsty PG）。详见 [APISIX 端口收敛与安全访问](../08-运维/APISIX端口收敛.md)。
- 环境差异注意：APISIX Admin Key、数据库密码、JWKS 全部经 `.env.<env>` 注入，任何非本地部署前必须更换默认值（如 `openssl rand -hex 16`）。

---

> 参考：[项目简介](项目概览.md) · [部署与环境配置](../03-部署指南/环境配置.md) · [PostgREST 使用指南](../06-API参考/PostgREST使用指南.md) · [网关路由](../06-API参考/网关路由.md) · [数据库迁移](../05-开发指南/数据库迁移.md) · [pgTAP 测试指南](../07-测试/pgTAP测试指南.md)