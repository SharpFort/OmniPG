...
| 审计页慢 | audit_log 无分区且增长快 | 见 [审计与日志](审计与日志.md) 保留/归档建议 |
...
Logto 事件 → APISIX /rpc/webhook_logto（HMAC 验签：logto-signature-sha-256）→ api_v1_platform.webhook_logto(jsonb) → sync_* 函数 → 镜像表（users / tenants / role / organization_role / user_tenants / user_role）。

### 4.2 排查入口：webhook_event_log

| RPC | 用途 |
| --- | --- |
| api_v1_platform.rpc_list_webhook_events(p_result, p_limit≤100, p_offset) | 查看 received/success/error/ignored 事件（仅超管） |
| api_v1_platform.rpc_replay_webhook_event(p_event_id uuid) | 重放 error 事件（仅超管），并记 log_operate |

```bash
# 查最近 50 条事件
curl -s 'http://localhost:9080/api/v1/platform/rpc/rpc_list_webhook_events' \
...
- 当前路由（7 条）：100 /.well-known/jwks（代理 Logto）· 95 POST /rpc/webhook_logto（HMAC 验签，无 jwt-auth）· 80 POST /rpc/ensure_user · 60 /logto/* · 50 /api/v1/platform/*（重写至 api_v1_platform）· 40 /rpc/* · 10 /*。api_v1_sales / api_v1_inventory 路由 2026-08-15 已退役。
...
# ② 路由清单（Logto 版 7 条：100 /.well-known/jwks、95 POST /rpc/webhook_logto、80 POST /rpc/ensure_user、60 /logto/*、50 /api/v1/platform/*、40 /rpc/*、10 /*）
curl -s http://localhost:9180/apisix/admin/routes -H "X-API-KEY: ${APISIX_ADMIN_KEY}"

# ③ 命中测试（带 token 与不带 token 对比）
curl -s -o /dev/null -w '%{http_code}' http://localhost:9080/api/v1/platform/role
curl -s -o /dev/null -w '%{http_code}' http://localhost:9080/api/v1/platform/role -H "Authorization: Bearer $TOKEN"
```
...
| 路径 404 但 /rpc/* 可用 | 路由优先级/rewrite 不对 | 检查 api_v1_platform 路由 regex_uri（^/api/v1/platform/(.*) → /$1） |
...
- **迁移前备份是硬要求**（见 [备份与恢复](备份与恢复.md)）；回滚首选从 pg_dump 快照恢复，而不是依赖 dbmate rollback。
...
> 参考：[部署指南总览](../03-部署指南/部署总览.md) · [数据流](../04-架构/数据流.md) · [网关路由](../06-API参考/网关路由.md) · [Logto Webhook](../06-API参考/LogtoWebhook接入.md) · [备份与恢复](备份与恢复.md) · [审计与日志](审计与日志.md)