...
| Logto | logto 库也在宿主 Pigsty PG 内（业务库走宿主 5433），随库备份覆盖（Pigsty 侧 logto 角色/库条目待补齐，见 [生产问题排查](生产问题排查.md)） |
...
数据库规模小、单实例，逻辑备份是最低成本的兜底：
    ...
    pg_dump -h 127.0.0.1 -U app_owner -d app_db -Fc \
      -t platform.audit_log -f backups/audit_log_$(date +%Y%m%d).dump
...
> 参考：[部署指南总览](../03-部署指南/部署总览.md) · [迁移指南](../05-开发指南/数据库迁移.md) · [冒烟验证脚本](../07-测试/冒烟验证脚本.md) · [生产问题排查](生产问题排查.md) · [安全设计](安全设计.md)